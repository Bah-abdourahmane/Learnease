    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox"
                    class="rounded bg-gray-300  border-primary text-primary shadow-sm focus:ring-primary"
                    name="remember">
        ...
        <div class="block mt-4">
            ...
            <div class="flex items-center my-5">
                <hr class="flex-1">
                <span class="text-white p-1">OR</span>
                <hr class="flex-1">
            </div>
            {{-- icons --}}
            <div class="flex items-center justify-center gap-7">
                @foreach (App\Enums\Provider::values() as $provider)
                    <a href="{{ route('oauth.redirect', ['provider' => $provider]) }}"
                        class="bg-indigo-50 rounded-full p-2">
